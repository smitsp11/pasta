import type { DemoData, RunEvent, RunResult, SessionStarted, StreamItem } from "../types";
...
export function buildScript(d: DemoData): Cue[] {
  ...
  for (const r of d.reads) { research.push({ t: tr, item: { type: "source_read", source_id: r.source_id, quotes: r.quotes, timestamp: iso(tr) } }); tr += T.research_gap; }
  ...
  for (let i = 1; i <= d.nodes.length; i++) {           // reveal nodes one at a time by re-sending the growing list
    site.push({ t: ts, item: { type: "site_model", site_model: d.site_model, nodes: d.nodes.slice(0, i) } }); ts += T.site_gap;
  }
  const merged = [...research, ...site].sort((a, b) => a.t - b.t);
  cues.push(...merged); t = Math.max(tr, ts);

  push(T.brief_after, { type: "brief", personas: d.personas, stress_tests: d.stress_tests });
  ...
  d.runs.forEach((run, i) => {
    const p = personaById[run.persona_id]; const cfg = p.config;
    ...
    const events: RunEvent[] = run.steps.map(([action, path], k) => ({ type: "run_event", run_id: run.run_id, session_id: run.session_id, journey_id: p.journey_id, config: cfg,
      step_index: k + 1, timestamp: iso(t0 + (k + 1) * T.step_gap), action, observation: /^https?:\/\//.test(path) ? path : `${origin}${path}`, screenshot_ref: null, outcome: "step_ok" }));
    ...
    runCues.push({ t: tEnd, item: { type: "run_result", run_id: run.run_id, journey_id: p.journey_id, config: cfg, session_id: run.session_id, outcome: run.outcome,
  ...
  push(T.score_after, { type: "result", scorecard: d.scorecard, findings: d.findings, affected: d.affected });
