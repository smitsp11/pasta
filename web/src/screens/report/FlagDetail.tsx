...
export function FlagDetail({ f, persona, sources, feed, position, total, where, onPrev, onNext, ref }: { f: Finding; persona: Persona | undefined; sources: Source[]; feed?: Feed; position: number; total: number; where: string; onPrev?: () => void; onNext?: () => void; ref?: React.Ref<HTMLDivElement> }) {
    ...
    <div ref={ref} tabIndex={-1} aria-label={`Issue ${position} of ${total}: ${flagTitle(f)}`} style={{ display: "flex", flexDirection: "column", gap: 20, minWidth: 0, outline: "none", scrollMarginTop: 80 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14, border: "1px solid rgba(18,122,62,0.4)", background: "rgba(31,157,85,0.07)", padding: 18 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
          <button type="button" onClick={() => setOpen(true)} className="btn-primary" style={{ fontFamily: "var(--font-mono)", fontSize: 12, fontWeight: 500, letterSpacing: "0.12em", color: "#FFFFFF", background: "var(--scan-deep)", border: "none", padding: "13px 22px", whiteSpace: "nowrap", cursor: "pointer" }}>OPEN REPLAY ↗</button>
