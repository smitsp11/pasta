    ...
    <motion.section layout initial={false} animate={{ minHeight: collapsed ? 0 : "calc(100vh - 64px)", paddingTop: collapsed ? 24 : 32, paddingBottom: collapsed ? 24 : 48 }}
      ...
      <div style={{ position: "relative", zIndex: 2, width: "100%", maxWidth: 660, display: "flex", flexDirection: "column", alignItems: "center", gap: collapsed ? 16 : 34 }}>
        ...
        <form style={{ position: "relative", zIndex: 1, width: "100%", maxWidth: 580, display: "flex", flexWrap: "wrap", gap: 10 }} onSubmit={e => { e.preventDefault(); onRun(url); }}>
          ...
          <button type="submit" className="btn-primary" style={{ flex: "0 0 auto", fontFamily: "var(--font-mono)", fontSize: 13, fontWeight: 500, letterSpacing: "0.14em", color: "#FFFFFF", background: "var(--scan-deep)", border: "none", padding: "15px 28px", cursor: "pointer", whiteSpace: "nowrap" }}>{collapsed ? "RUN AGAIN →" : "RUN IRIS →"}</button>
        </form>
      </div>
    </motion.section>);
}
