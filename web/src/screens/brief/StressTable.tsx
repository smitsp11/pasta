...
const head = (extra: React.CSSProperties): React.CSSProperties => ({ fontFamily: "var(--font-mono)", fontSize: 10, letterSpacing: "0.14em", color: "var(--muted)", padding: "10px 12px", borderBottom: "1px solid rgba(20,20,20,0.1)", ...extra });
const cell = (last: boolean, muted = false, extra: React.CSSProperties = {}): React.CSSProperties => ({ fontFamily: "var(--font-mono)", fontSize: 11, lineHeight: 1.5, color: muted ? "var(--muted)" : "#141414", padding: "11px 12px", borderBottom: last ? "none" : "1px solid rgba(20,20,20,0.08)", ...extra });
export function StressTable({ rows }: { rows: StressTest[] }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, fontWeight: 500, letterSpacing: "0.18em", color: "var(--muted)" }}>STRESS TESTS</span>
