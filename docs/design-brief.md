...
Layout: full-viewport hero on the dot-field background, as minimal as it can be. Centre: serif headline, one wide input, one `scan` button. Nothing else on the page except the top bar and, behind the form at 30% opacity, a dithered illustration of a retro computer whose screen shows a dot-matrix globe (the Browserbase computer, ours).

Copy: headline *"Send your customers in first."* · input placeholder `https://northwindoutfitters.com` · button `Run Iris`.

Not on this screen: no sub-line, no optional context field, no corner annotations. The operator hint for the stage fallback is a URL parameter (`?hint=`), never a visible field.
...
**Frame 1 prompt:**
> Frame 1, "Input". Full-viewport hero on the dithered dot-field background (dense ink dots at the top fading to plain paper behind the form). As minimal as possible: only the top bar, a centred serif headline "Send your customers in first.", one wide input with placeholder https://northwindoutfitters.com, and one orange button "Run Iris". No sub-line, no second field, no corner annotations. Behind the form at 30% opacity, a Bayer-dithered illustration of a retro desktop computer (Browserbase style) whose screen shows a dot-matrix globe. Stage bar shows LEARN as upcoming.
